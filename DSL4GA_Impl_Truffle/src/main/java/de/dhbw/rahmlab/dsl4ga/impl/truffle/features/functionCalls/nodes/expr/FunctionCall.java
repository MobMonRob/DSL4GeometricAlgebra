...
public abstract class FunctionCall extends ExpressionBaseNode {
	...
	@Specialization
	protected MultivectorSymbolic call(VirtualFrame frame, @CachedLibrary(limit = "2") InteropLibrary library) {
	...
	protected CgaListTruffleBox doExecuteArguments(VirtualFrame frame) {
		...
		MultivectorSymbolic[] argumentValues = new MultivectorSymbolic[this.arguments.length];
	...
	protected MultivectorSymbolic doExecuteFunction(Function function, CgaListTruffleBox argumentValueBoxed, InteropLibrary library) throws InterpreterInternalException {
		try {
			...
			if (returnValue instanceof CgaTruffleBox) {
				...
				List<MultivectorSymbolic> mvecs = ((CgaListTruffleBox) returnValue).getInner();
